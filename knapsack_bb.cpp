...
void knapsack(int W, int wt[], int val[], int k, int n, int hiican_val) {
	...
	if (k + 1 == n) {
		upper_bound_left = hiican_val;
		upper_bound_right = val[k] + hiican_val;
	} else {
		upper_bound_left=hiican_val+W*(val[k+1]/wt[k+1]);
		upper_bound_right=hiican_val+val[k]+(W-wt[k])*(val[k+1]/wt[k+1]);
	}
